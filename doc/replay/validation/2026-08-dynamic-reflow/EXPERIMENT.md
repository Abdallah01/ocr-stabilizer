...
(Corrected 2026-09-02 — see the #136 entry's Reproduce section: the
non-reproduction below was a line-ending artefact; the Tesseract streams
DO regenerate byte for byte.) The numbers above come from the COMMITTED streams, not from regenerating
...
uses the seed itself for the noise; repetition 2 uses seed + 1000.
...
knobs. A configuration's step and control pages are the same page: both
generators build the same paragraphs from the same seed. The PaddleOCR
...
|---|---|---|---|
| s93-r1 | 93 | 93 | nothing — the published streams themselves (its three controls are the tesseract-matrix originals at seed 94, a different page) |
| s93-r2 | 93 | 1093 | the published page under fresh noise |
| s07-r1 | 7 | 7 | a new page |
| s07-r2 | 7 | 1007 | s07's page under fresh noise |
| s21-r1 | 21 | 21 | a new page |
| s21-r2 | 21 | 1021 | s21's page under fresh noise |
| s42-r1 | 42 | 42 | a new page |
...
pixel above). The **largest firing floor** of a stream is the largest
...
364 px (median 335). The shipped 390 px floor therefore never fires on
...
published page the window is real in both noise repetitions — (377,
406] and (335, 392] — and 390 sits inside both, with 2 px to spare in
the second. On the s07 page the window is (220, 340] / (< 200, 346]:
...
the bounds by 0–42 px (s93 ceiling 377 to 335, slab 406 to 392; s07 slab
340 to 346; s21 ceiling 244 to 261; s42 slab 240 to 240) and, on the
published page, decides whether the 150 px step gets a plan at all. A
different page moves them by 100 px and more, and decides whether the
slab has a surviving mover. Both sources of variance are larger than the
29 px width of the window the #119 entry calibrated against.
...
scrollY 800, the same slab sizes and positions). What varies is the text
and the capture noise, so the numbers measure variance across pages and
capture sessions of ONE layout — the pixel bounds transfer to no other
...
searched at 1 px resolution over [200, 700] and assume monotonicity
(lowering the floor only adds qualified movers; extra movers outside the
winning cluster stay on damp) — the pin test checks each bound as a
boundary, not the assumption. The step rule is the #116 rule (lag at
