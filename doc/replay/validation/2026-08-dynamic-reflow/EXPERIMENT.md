...
The numbers above come from the COMMITTED streams. When this entry was
written, regenerated `pushdown.jsonl`/`rewrap.jsonl` were believed not
to match the committed bytes and the mismatch was attributed to
environment/version drift; the #136 entry (2026-09-02, its "Correction
to the #116 entry's Reproduce note") found that comparison was made
against CRLF working-tree copies — against the index bytes, Tesseract
5.4.0.20240606 + `chi_sim` + Microsoft YaHei + Pillow 12.2 regenerate
every committed Tesseract stream byte for byte. The `gen_corpus.py`
invocations below are the knobs each variant was produced with and,
with that toolchain, reproduce it; the PaddleOCR matrix streams were
not re-tested, so for those only the `ab-report` commands over the
committed files are the reproducible part.

```
# provenance; reproducible byte for byte with the toolchain named in the #136 entry's correction below
# (compare against the index bytes — the working tree is CRLF, the generator writes LF)
...
## Variance across seeds and repetitions (#136, 2026-09-02)

Every number above comes from ONE synthetic seed (93) and ONE repetition
per stream configuration, and two of those numbers carry the whole floor
result: the 377 px control maximum and the 406 px slab mover that bound
the `coherentShiftFloorPx` window. This entry regenerates the Tesseract
half of the corpus at three further seeds, adds a second repetition of
every seed (the published one included), and re-derives the tables above
per configuration.

### Method

A **configuration** is a (seed, perturb-seed) pair. The seed builds the
page — the paragraphs and their layout — and, by default, also drives
the per-frame capture noise (sub-pixel shift, JPEG round-trip,
brightness), exactly as the committed streams were made. `--perturb-seed`
(new on both generators) seeds the noise SEPARATELY, so a second
repetition is the SAME page under fresh noise — what a second capture
session of one page looks like to the engine. Repetition 1 of a seed
passes no `--perturb-seed`: its noise continues the page RNG where the
text left it, as the committed streams' did (each stream's meta note
stamps `seed S`); repetition 2 passes `--perturb-seed <seed + 1000>`, a
fresh generator (stamped `seed S perturb-seed S+1000`). The stamp names
exactly the flags that regenerate the stream.

Each configuration holds the eleven Tesseract streams of the tables
above — the seven step streams, `rewrap`, and the three tesseract-matrix
controls — generated by `variants/gen_seed_suite.py` with the committed
knobs. In the seven regenerated configurations the step and control
pages are the same page: both generators build the same paragraphs from
the same seed. The published configuration (s93-r1) is the exception:
its three controls are the tesseract-matrix originals, built at that
generator's default seed 94 — a different page from the seed-93 step
page — so its window pairs a seed-94 control ceiling with a seed-93
slab bound, and s93-r2 (seed 93 throughout) is the first configuration
whose ceiling and bound come from one page. The PaddleOCR
and ML Kit controls cannot be regenerated by this rig (an engine and a
device it does not have), so every per-configuration tally below is over
the 11-stream Tesseract subset; on that subset the published corpus
scores 4/4 controls and 4/7 steps under today's `coherentShift`.

| config | seed | perturb-seed | what varies against the published corpus |
|---|---|---|---|
| s93-r1 | 93 | — | nothing — the published streams themselves (its three controls are the tesseract-matrix originals at seed 94, a different page; no stamp) |
| s93-r2 | 93 | 1093 | the published step page under fresh noise; its three controls are the seed-93 page's, not the published seed-94 controls |
| s07-r1 | 7 | — | a new page |
| s07-r2 | 7 | 1007 | s07's page under fresh noise |
| s21-r1 | 21 | — | a new page |
| s21-r2 | 21 | 1021 | s21's page under fresh noise |
| s42-r1 | 42 | — | a new page |
| s42-r2 | 42 | 1042 | s42's page under fresh noise |

Every cell below is a live replay of the committed streams
(`variants/variance_report.py` prints these tables;
`test/replay/experiment_doc_variance_tables_test.dart` pins them, the
floor bounds as the boundary they claim — fires at the bound, not one
pixel above — and the seed / perturb-seed cells above against every
stream's own meta-note stamp). The **largest firing floor** of a stream is the largest
integer floor at which `--coherent-floor=N` still produces a step event
on it: the stream's largest floor-qualified displacement at 1 px
resolution, found by bisection in [200, 700] ("< 200" = never fires in
range). On the published seed it recovers the 377 px (`tess-scroll`) and
406 px (`pushdown-600`) figures the #119 entry read from an instrumented
print, and every lag triple and tally of that entry.

### Controls (4 per configuration)

| config | control | coherent stepEvents | floor 390 stepEvents | adopt stepEvents | largest firing floor (px) |
|---|---|---|---|---|---|
| s93-r1 | rewrap | 0 | 0 | 0 | 339 |
| s93-r1 | tess-stable-dwell | 0 | 0 | 0 | < 200 |
| s93-r1 | tess-jitter-dwell | 0 | 0 | 0 | < 200 |
| s93-r1 | tess-scroll | 0 | 0 | 0 | 377 |
| s93-r2 | rewrap | 0 | 0 | 0 | 335 |
| s93-r2 | tess-stable-dwell | 0 | 0 | 0 | < 200 |
| s93-r2 | tess-jitter-dwell | 0 | 0 | 0 | < 200 |
| s93-r2 | tess-scroll | 0 | 0 | 0 | 208 |
| s07-r1 | rewrap | 0 | 0 | 0 | 220 |
| s07-r1 | tess-stable-dwell | 0 | 0 | 0 | < 200 |
| s07-r1 | tess-jitter-dwell | 0 | 0 | 0 | < 200 |
| s07-r1 | tess-scroll | 0 | 0 | 0 | < 200 |
| s07-r2 | rewrap | 0 | 0 | 0 | < 200 |
| s07-r2 | tess-stable-dwell | 0 | 0 | 0 | < 200 |
| s07-r2 | tess-jitter-dwell | 0 | 0 | 0 | < 200 |
| s07-r2 | tess-scroll | 0 | 0 | 0 | < 200 |
...
| s42-r2 | tess-scroll | 0 | 0 | 0 | 359 |

### Step streams (7 per configuration)

| config | stream | move cap | damp lag move/+3/+5 | coherent lag move/+3/+5 (stepEvents) | adopt lag move/+3/+5 (stepEvents) | floor 390 lag move/+3/+5 (stepEvents) | step rule: coherent / adopt / floor |
|---|---|---|---|---|---|---|---|
...
| s07-r1 | pushdown-150 | 7 | 73.8 / 70.6 / 51.1 | 73.8 / 70.6 / 51.1 (0) | 73.8 / 70.6 / 51.1 (0) | 73.8 / 70.6 / 51.1 (0) | FAIL / FAIL / FAIL |
| s07-r1 | pushdown-300 | 7 | 96.8 / 61.7 / 67.1 | 2.0 / 1.4 / 12.3 (7) | 2.0 / 1.4 / 12.3 (7) | 2.0 / 1.4 / 12.3 (7) | PASS / PASS / PASS |
| s07-r1 | pushdown-600 | 7 | 24.9 / 13.5 / 13.1 | 24.9 / 13.5 / 13.1 (0) | 24.9 / 13.5 / 13.1 (0) | 24.9 / 13.5 / 13.1 (0) | FAIL / FAIL / FAIL |
| s07-r1 | pushup-300 | 7 | 138.0 / 67.4 / 57.0 | 7.5 / 7.9 / 5.5 (11) | 7.5 / 7.9 / 5.5 (11) | 7.5 / 7.9 / 5.5 (11) | PASS / PASS / PASS |
| s07-r1 | pushdown-300-early | 3 | 79.4 / 34.7 / 29.3 | 2.5 / 1.9 / 2.3 (7) | 2.5 / 1.9 / 2.3 (7) | 2.5 / 1.9 / 2.3 (7) | PASS / PASS / PASS |
| s07-r1 | pushdown-300-late | 10 | 99.7 / n/a / n/a | 1.5 / n/a / n/a (7) | 1.5 / n/a / n/a (7) | 1.5 / n/a / n/a (7) | PASS / PASS / PASS |
| s07-r2 | pushdown-050 | 7 | 26.7 / 24.0 / 22.3 | 26.7 / 24.0 / 22.3 (0) | 26.7 / 24.0 / 22.3 (0) | 26.7 / 24.0 / 22.3 (0) | FAIL / FAIL / FAIL |
| s07-r2 | pushdown-150 | 7 | 74.5 / 61.8 / 52.7 | 74.5 / 61.8 / 52.7 (0) | 74.5 / 61.8 / 52.7 (0) | 74.5 / 61.8 / 52.7 (0) | FAIL / FAIL / FAIL |
| s07-r2 | pushdown-300 | 7 | 109.4 / 63.1 / 56.8 | 8.7 / 6.4 / 8.1 (7) | 8.7 / 6.4 / 8.1 (7) | 8.7 / 6.4 / 8.1 (7) | PASS / PASS / PASS |
| s07-r2 | pushdown-600 | 7 | 22.8 / 22.8 / 13.1 | 22.8 / 22.8 / 13.1 (0) | 22.8 / 22.8 / 13.1 (0) | 22.8 / 22.8 / 13.1 (0) | FAIL / FAIL / FAIL |
| s07-r2 | pushup-300 | 7 | 144.4 / 70.0 / 60.3 | 8.3 / 7.9 / 6.8 (11) | 8.3 / 7.9 / 6.8 (11) | 8.3 / 7.9 / 6.8 (11) | PASS / PASS / PASS |
| s07-r2 | pushdown-300-early | 3 | 90.9 / 43.5 / 36.4 | 8.3 / 7.7 / 6.4 (7) | 8.3 / 7.7 / 6.4 (7) | 8.3 / 7.7 / 6.4 (7) | PASS / PASS / PASS |
| s07-r2 | pushdown-300-late | 10 | 100.2 / n/a / n/a | 1.5 / n/a / n/a (7) | 1.5 / n/a / n/a (7) | 1.5 / n/a / n/a (7) | PASS / PASS / PASS |
| s21-r1 | pushdown-050 | 7 | 24.1 / 23.2 / 20.9 | 24.1 / 23.2 / 20.9 (0) | 24.1 / 23.2 / 20.9 (0) | 24.1 / 23.2 / 20.9 (0) | FAIL / FAIL / FAIL |
| s21-r1 | pushdown-150 | 7 | 69.2 / 57.5 / 54.6 | 69.2 / 57.5 / 54.6 (0) | 69.2 / 57.5 / 54.6 (0) | 69.2 / 57.5 / 54.6 (0) | FAIL / FAIL / FAIL |
| s21-r1 | pushdown-300 | 7 | 111.8 / 70.8 / 64.0 | 11.5 / 7.3 / 7.6 (9) | 11.5 / 7.3 / 7.6 (9) | 11.5 / 7.3 / 7.6 (9) | PASS / PASS / PASS |
| s21-r1 | pushdown-600 | 7 | 2.3 / 12.7 / 12.5 | 2.3 / 12.7 / 12.5 (0) | 2.3 / 12.7 / 12.5 (0) | 2.3 / 12.7 / 12.5 (0) | FAIL / FAIL / FAIL |
| s21-r1 | pushup-300 | 7 | 93.8 / 46.5 / 40.6 | 2.3 / 13.1 / 13.1 (6) | 2.3 / 13.1 / 13.1 (6) | 2.3 / 13.1 / 13.1 (6) | PASS / PASS / PASS |
| s21-r1 | pushdown-300-early | 3 | 89.4 / 47.0 / 40.5 | 13.4 / 12.9 / 2.9 (9) | 13.4 / 12.9 / 2.9 (9) | 13.4 / 12.9 / 2.9 (9) | PASS / PASS / PASS |
| s21-r1 | pushdown-300-late | 10 | 112.9 / n/a / n/a | 8.8 / n/a / n/a (9) | 8.8 / n/a / n/a (9) | 8.8 / n/a / n/a (9) | PASS / PASS / PASS |
| s21-r2 | pushdown-050 | 7 | 23.7 / 23.3 / 23.0 | 23.7 / 23.3 / 23.0 (0) | 23.7 / 23.3 / 23.0 (0) | 23.7 / 23.3 / 23.0 (0) | FAIL / FAIL / FAIL |
| s21-r2 | pushdown-150 | 7 | 67.9 / 54.0 / 53.4 | 67.9 / 54.0 / 53.4 (0) | 67.9 / 54.0 / 53.4 (0) | 67.9 / 54.0 / 53.4 (0) | FAIL / FAIL / FAIL |
| s21-r2 | pushdown-300 | 7 | 102.2 / 66.7 / 54.8 | 8.2 / 7.0 / 7.9 (8) | 8.2 / 7.0 / 7.9 (8) | 8.2 / 7.0 / 7.9 (8) | PASS / PASS / PASS |
| s21-r2 | pushdown-600 | 7 | 0.5 / 12.2 / 2.6 | 0.5 / 12.2 / 2.6 (0) | 0.5 / 12.2 / 2.6 (0) | 0.5 / 12.2 / 2.6 (0) | FAIL / FAIL / FAIL |
| s21-r2 | pushup-300 | 7 | 109.4 / 48.7 / 43.9 | 8.0 / 4.4 / 5.2 (6) | 8.0 / 4.4 / 5.2 (6) | 8.0 / 4.4 / 5.2 (6) | PASS / PASS / PASS |
| s21-r2 | pushdown-300-early | 3 | 83.9 / 42.5 / 36.0 | 6.4 / 8.7 / 6.8 (8) | 6.4 / 8.7 / 6.8 (8) | 6.4 / 8.7 / 6.8 (8) | PASS / PASS / PASS |
| s21-r2 | pushdown-300-late | 10 | 103.3 / n/a / n/a | 0.2 / n/a / n/a (8) | 0.2 / n/a / n/a (8) | 0.2 / n/a / n/a (8) | PASS / PASS / PASS |
| s42-r1 | pushdown-050 | 7 | 23.1 / 22.6 / 21.7 | 23.1 / 22.6 / 21.7 (0) | 23.1 / 22.6 / 21.7 (0) | 23.1 / 22.6 / 21.7 (0) | FAIL / FAIL / FAIL |
...
### The floor window, per configuration

The **control ceiling** is the largest firing floor over the four
controls; the **slab bound** is `pushdown-600`'s. The window is
(ceiling, bound]: a floor inside it fires on the slab and on no control.

| config | control ceiling (px) | slab bound (px) | floor window | 390 inside | pushdown-600 lag at move: coherent → floor 390 |
|---|---|---|---|---|---|
| s93-r1 | 377 | 406 | (377, 406] | yes | 30.7 → 1.4 |
| s93-r2 | 335 | 392 | (335, 392] | yes | 30.1 → 0.1 |
| s07-r1 | 220 | 340 | (220, 340] | no | 24.9 → 24.9 |
...
|---|---|---|---|---|
| control ceiling (px) | 220 | 335 | 377 | 7 |
| slab bound (px) | 240 | 343 | 406 | 6 |
| window width (px) | 29 | 57 | 120 | 3 |
| steps passing: coherent (of 7) | 4 | 4 | 4 | 8 |
| steps passing: adopt (of 7) | 4 | 4 | 5 | 8 |
| steps passing: floor 390 (of 7) | 4 | 4 | 4 | 8 |
| control false events at floor 390 (4 controls) | 0 | 0 | 0 | 8 |

### Reading

Four results, in decreasing order of how well they held up.

**1. The controls hold under every lever, on every configuration.** All
32 control replays report 0 step events for `coherentShift`, for the
...
**2. `coherentShift`'s 4/7 is stable.** Every configuration passes and
fails the same four and three step streams: the 300 px class in all four
positions passes (lag at the move 0.1–22.1 px against damp's 70–169 px),
the 50 px step stays inside the jitter allowance, the 150 px and 600 px
steps fail. Step-event counts on the passing streams vary 5–11 with the
page, but no configuration flips a verdict.

**3. The 150 px blind spot is the rule; candidate 3 closes it on one
configuration only.** The adopt lever's 16 events and 6.0 px lag on
`pushdown-150` are the published seed's noise draw: on the same page
under fresh noise (s93-r2) and on all six configurations of the three
other pages, NO matched pair clears its own 3x-height gate on the move
capture, so no plan exists to follow and every arm is byte-identical to
damp (0 events). `coherentShiftAdoptAgreeing: true` remains the right
default — it changed nothing on any control or on any step where no
plan formed, exactly as candidate 3's mechanism predicts — but its
measured benefit is one configuration in eight, and the "closes the
150 px blind spot" wording should not be read as a property of the
lever. The 150 px step is a property of the jitter allowance: the
quorum's "moved" gate (3x own height, 108–186 px on these lines) sits
right where a 150 px step lands.

**4. The floor window is page-specific, and often empty.** On the
published streams the window is real — (377, 406], its ceiling from
the seed-94 control page — and so is the seed-93 page's under fresh
noise, (335, 392], measured on one page throughout; 390 sits inside
both, with 2 px to spare in the second. On the s07 page the window is (220, 340] / (< 200, 346]:
390 misses it, and a floor near 300 would have worked there. On the s21
page no floor-qualified mover survives at any floor from 200 up (every
shifted line is re-admitted as a new identity — which is also why damp's
...
the four synthetic pages (0 control events, the slab untouched). The
README's calibration recipe stands — measure both bounds on your own
captures, leave the floor `null` when the window is empty — and its
"377 / 406 / 390" figures are now labelled as the published page's, not
the corpus's.
...
plan at all. The published configuration's ceiling pair, 377 to 335,
is NOT a same-page comparison: 377 is the seed-94 control page's
`tess-scroll`, 335 the seed-93 page's `rewrap`. A different page moves
the bounds by 100 px and more, and decides whether the slab has a
surviving mover. The within-page moves are of the order of the 29 px
width of the window the #119 entry calibrated against; the
between-page moves exceed it several times over.

### Boundary of what this proves

Three further seeds of the SAME generator: the same vocabulary and
sentence grammar, the same geometry (1080 px page, 36 px font, 56 px line
height, 26 characters per line, a 2200 px viewport dwelling at
scrollY 800, the same slab sizes, inserted after the same paragraph).
The slab's page position follows each seed's text — page y 1692 on the
published page, 1804 on s07, 1860 on s21 and s42 (each stream's meta
note records it) — so how much pushed content the viewport keeps below
the slab differs by up to 168 px between pages. What varies is the
text, the slab's position with it, and the capture noise, so the
numbers measure variance across pages and capture sessions of ONE
layout — the pixel bounds transfer to no other
font size, line height or capture cadence, which is the calibration
recipe's point. Two repetitions per page (n = 2 for any within-page
statement) and four pages (n = 4 across pages; n = 8 configurations
overall). The controls are the four Tesseract streams only: the six
PaddleOCR / ML Kit controls of the #119 tables were not regenerated, so
their zero remains a single-configuration result. The floor bounds are
searched at 1 px resolution over [200, 700] by bisection, which assumes
monotonicity: a stream that fires at F fires at every floor below F. The
engine does not guarantee it — lowering the floor only adds qualified
movers, but the floor path direction-checks the WHOLE qualified set
before clustering, so an added mover travelling against the slab vetoes
the plan outright. The pin test checks each bound as a bracket (fires
at the bound and at 200, not one pixel above the bound) and each
"< 200" / "none" as silence at five floors across the range — on all
40 control and slab cells those probes found no reversal — but not the
assumption itself. The step rule is the #116 rule (lag at
most half of damp's at the move, +3 and +5 over the captures the stream
has); identity at the move is not reported (it never discriminated).
Nothing here measures device timing.

### Reproduce

```
# each configuration ~3 minutes of Tesseract; the toolchain of the correction below
# regenerates the committed streams byte for byte, so these are re-derivable, not provenance-only
python doc/replay/validation/2026-08-dynamic-reflow/variants/gen_seed_suite.py <tesseract.exe> doc/replay/validation/2026-08-dynamic-reflow/variants --seed 93 --perturb-seed 1093 --label s93-r2
python doc/replay/validation/2026-08-dynamic-reflow/variants/gen_seed_suite.py <tesseract.exe> doc/replay/validation/2026-08-dynamic-reflow/variants --seed 7 --label s07-r1
python doc/replay/validation/2026-08-dynamic-reflow/variants/gen_seed_suite.py <tesseract.exe> doc/replay/validation/2026-08-dynamic-reflow/variants --seed 7 --perturb-seed 1007 --label s07-r2
python doc/replay/validation/2026-08-dynamic-reflow/variants/gen_seed_suite.py <tesseract.exe> doc/replay/validation/2026-08-dynamic-reflow/variants --seed 21 --label s21-r1
python doc/replay/validation/2026-08-dynamic-reflow/variants/gen_seed_suite.py <tesseract.exe> doc/replay/validation/2026-08-dynamic-reflow/variants --seed 21 --perturb-seed 1021 --label s21-r2
python doc/replay/validation/2026-08-dynamic-reflow/variants/gen_seed_suite.py <tesseract.exe> doc/replay/validation/2026-08-dynamic-reflow/variants --seed 42 --label s42-r1
python doc/replay/validation/2026-08-dynamic-reflow/variants/gen_seed_suite.py <tesseract.exe> doc/replay/validation/2026-08-dynamic-reflow/variants --seed 42 --perturb-seed 1042 --label s42-r2

# the tables above, from the committed streams (deterministic; about a minute per configuration)
python doc/replay/validation/2026-08-dynamic-reflow/variants/variance_report.py
```

**Correction to the #116 entry's Reproduce note.** The claim that "this
environment's Tesseract/PaddleOCR toolchain does not reproduce
`pushdown.jsonl`/`rewrap.jsonl` bit-for-bit" was a line-ending artefact:
the generators write LF, git checks the streams out with CRLF on Windows,
and the comparison was made against the working-tree copy. Compared
against the index bytes (`git show HEAD:<path>`), Tesseract
5.4.0.20240606 + `chi_sim` (tessdata_fast) + Microsoft YaHei + Pillow
12.2 regenerate all five committed Tesseract streams byte for byte —
every block text, rect and confidence (the tesseract-matrix generator's
meta line had its keys in a different order from the committed files;
aligned in this entry). The `gen_corpus.py` lines above marked
"provenance only" are reproducible after all; the PaddleOCR streams were
not re-tested.
