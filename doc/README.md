...
| [`replay/validation/2026-08-dynamic-reflow/`](replay/validation/2026-08-dynamic-reflow/) | **Tesseract 5, dynamic reflow (#93).** Two synthesized non-rotation reflows — an image slab pushing every line below it down 300 px, and a font swap re-wrapping every line — with the regime each lands in stated (push-down: identity kept, position lags, #116; re-wrap: identity reset), a unit-of-identity addendum (the same streams replayed as lines and as pre-grouped paragraphs — grouping before tracking imports its own instability into identity), and pinned by a test. Fully regenerable. **`variants/` (#116 step-response A/B, 2026-08-29):** six further `gen_corpus.py` streams grading `StepResponse.snap`/`.coherentShift` against the `agreementWeighted` (damp) baseline — `pushdown-050`/`-150`/`-600` = `--gap-px 50/150/600 --reflow-at 7`; `pushup-300` = `--gap-px -300 --reflow-at 7` (content moves UP); `pushdown-300-early`/`-late` = `--gap-px 300 --reflow-at 3/10`; seed 93 throughout. Committed as `.jsonl` + `.ab.json`: the committed streams are the numbers' source of truth, and the #136 entry's Reproduce note names the toolchain (Tesseract 5.4.0 + `chi_sim` + Microsoft YaHei + Pillow 12.2) that regenerates every committed Tesseract stream byte for byte (an earlier "version drift" note was a line-ending artefact; the PaddleOCR streams were not re-tested). Full 17-stream table and verdict: EXPERIMENT.md's "Step response A/B" section. **`variants/<config>/` (#136 seed variance, 2026-09-02):** 77 further streams — the same eleven Tesseract streams on three more seeds, each with a fresh-noise repetition, plus a fresh-noise repetition of the published seed (`--perturb-seed`); `.jsonl` only, replayed live by `variants/variance_report.py` and pinned by `test/replay/experiment_doc_variance_tables_test.dart`. Verdict: EXPERIMENT.md's "Variance across seeds and repetitions" section. |
