...
List<DefaultTrackedBlock<void>> _page({double scale = 1.0, double dy = 0}) => [
      for (var i = 0; i < 4; i++)
        _at(
          Rect.fromLTWH(
              40 * scale, (500 + 100.0 * i) * scale + dy, 300 * scale, 20 * scale),
          _texts[i],
        ),
    ];

void main() {
  test('a pure zoom about the page origin reports scale k, ~zero '
  ...
  test('a coherent translation (+150 px step) reports scale ~1 and the '
  ...
  test('the estimate exists under damp and snap too — it is collected in '
  ...
  test('a session\'s first sighting (nothing cached) and a capture with '
  ...
  test('a carousel-child fresh block is not a pair: its match merges but '
  ...
  test('a provisional cached block (a band admission still inside its '
  ...
  test('transformEstimateMinPairs below 2 is rejected at construction', () {
    expect(() => _engine(minPairs: 1), throwsArgumentError);
    expect(() => _engine(minPairs: 2), returnsNormally);
  });
}
