...
is returned usable from its **first** observation; re-observations only
refine its position (see [Timing model](#timing-model)).
...
- `stabilize()` returns first-sighting blocks in `stableBlocks` on the very
  call that observed them. Nothing is withheld while evidence accrues.
...
- `wellObservedTexts` fires at **3** observations. It is a hint that a
  translation can be cached long-term — not a display gate.
- The refinement supply is demand-coupled: re-observations and jitter both
  come from captures. A stream that produces no re-observations also
  produces no wobble to damp; whenever the wobble exists, so does the
  evidence that suppresses it.
